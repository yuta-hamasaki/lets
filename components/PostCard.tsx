...
export const PostCard: React.FC<PostCardProps> = ({ post, onViewProfile }) => {
    ...
    const handleProfileClick = () => {
        if (onViewProfile && post.authorId) {
        ...
        <article className="bg-white rounded-3xl p-5 shadow-sm border border-slate-200">
            <div className="flex items-center gap-3 mb-3">
                <div 
                    ...
                    style={{ backgroundImage: author?.avatar ? `url(${author.avatar})` : 'none' }} 
                    onClick={handleProfileClick}
                    role="button"
                    aria-label={`${post.author}のプロフィールを表示`}
                />
                <div 
                    className="flex flex-col cursor-pointer"
                    onClick={handleProfileClick}
                >
                    <div className="font-extrabold text-slate-800 text-base">{post.author}</div>
                    <div className="text-[0.85rem] text-slate-400 mt-px">{post.timestamp}</div>
                </div>
            </div>
            ...
            <div className="flex items-center pt-3 border-t border-slate-50 gap-6">
                <div className="flex items-center gap-2 text-slate-400 text-[0.9rem] font-semibold cursor-pointer">
                    ...
                    <span className="text-[0.9rem] font-bold">{post.likes}</span>
                </div>
                <div className="flex items-center gap-2 text-slate-400 text-[0.9rem] font-semibold cursor-pointer">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 1 1-7.6-14 8.38 8.38 0 0 1 3.8.9L21 3z"></path>
                    </svg>
                    <span className="text-[0.9rem] font-bold">{post.comments}</span>
                </div>
